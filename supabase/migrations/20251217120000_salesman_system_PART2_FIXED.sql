-- PART 2: Create Tables and Policies (FIXED VERSION - Can be re-run safely)
-- Make sure Part 1 completed successfully before running this
...
DROP POLICY IF EXISTS "Salesmen can view their assigned brands" ON public.salesman_brands;
CREATE POLICY "Salesmen can view their assigned brands"
ON public.salesman_brands FOR SELECT
USING (auth.uid() = salesman_id);

DROP POLICY IF EXISTS "Admins can manage salesman brands" ON public.salesman_brands;
...
DROP POLICY IF EXISTS "Salesmen can create orders for their clients" ON public.orders;
...
-- Drop existing policies if they exist, then create them
DROP POLICY IF EXISTS "Salesmen can view their own activity logs" ON public.salesman_activity_logs;
CREATE POLICY "Salesmen can view their own activity logs"
ON public.salesman_activity_logs FOR SELECT
USING (auth.uid() = salesman_id);

DROP POLICY IF EXISTS "Admins can view all salesman activity logs" ON public.salesman_activity_logs;
...
DROP POLICY IF EXISTS "System can insert activity logs" ON public.salesman_activity_logs;
