        ...
        return (
            <div className="min-h-screen bg-[#FDFCF9] flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    <div className="w-12 h-12 border-4 border-[#2D5F3F] border-t-transparent rounded-full animate-spin" />
                    <p className="text-[#6B6B6B] font-medium animate-pulse">Assigning Operator Credentials...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-6 border-b border-[#E8E8E8]">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-[10px] uppercase tracking-widest text-[#2D5F3F] font-bold">
                                    Operations Desk
                                </p>
                            </div>
                            <Button
                    ...
                    <nav className="flex-1 overflow-y-auto p-4 space-y-1 mt-4">
                        {navigation.map((item) => {
                            ...
                            const isActive = pathname === item.href || (item.href !== "/sub-admin" && pathname.startsWith(item.href));
                            return (
                                <Link key={item.name} href={item.href}>
                                    <Button
                                        ...
                                        className={cn(
                                            ...
                                            isActive
                                                ? "bg-[#F7F5F2] text-[#1A1A1A] font-semibold border-r-4 border-r-[#2D5F3F] rounded-r-none"
                                                : "text-[#6B6B6B] hover:text-[#1A1A1A] hover:bg-[#F7F5F2]/50"
                                        )}
                                        onClick={() => setSidebarOpen(false)}
                                    >
                                        <Icon className={cn("h-5 w-5", isActive ? "text-[#2D5F3F]" : "text-[#A0A0A0]")} />
                                        {item.name}
                                        {isActive && <ChevronRight className="h-4 w-4 ml-auto text-[#2D5F3F]" />}
                                    </Button>
                                </Link>
                            );
                        })}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 lg:gap-4">
                            <div className="hidden sm:flex flex-col items-end mr-2">
                                <p className="text-xs font-bold text-[#1A1A1A]">Sub-Admin Panel</p>
                                <p className="text-[10px] text-[#2D5F3F] flex items-center gap-1">
                                    <span className="w-1.5 h-1.5 bg-[#2D5F3F] rounded-full animate-pulse" />
                                    Operator Active
                                </p>
                            </div>
                            <Button variant="ghost" size="icon" className="relative text-[#6B6B6B] hover:text-[#2D5F3F]">
                    ...
                    {navigation.map(item => {
                        ...
                        return (
                            <Link key={item.name} href={item.href} className="flex flex-col items-center gap-1 px-3 py-1">
                                <Icon className={cn("h-5 w-5", isActive ? "text-[#2D5F3F]" : "text-[#6B6B6B]")} />
                                <span className={cn("text-[10px] font-medium", isActive ? "text-[#2D5F3F]" : "text-[#6B6B6B]")}>{item.name.split(' ')[0]}</span>
                            </Link>
                        )
                    })}
                </div>
